...
#             if self.split == 'train':
#                 self.img_info = self.img_info.iloc[:int(.9*len(self.img_info))]
...
#                 for im_ch in range(0,int(0.9*len(im_list))): # np.random.choice(len(im_list), 1)[0]
...
#                 for im_ch in range(int(0.9*len(im_list)),len(im_list)): # np.random.choice(len(im_list), 1)[0]
...
#                 if (i+1) % 100 == 0:
#                     print("processing: {} identities for {}".format(i+1, self.split))
#                 if upper and i == upper - 1:  # for debug purpose
#                     break
        
#         with open(self.image_list_file, 'r') as f:
#             for i, img_file in enumerate(f):
#                 img_file = split +'/'+img_file.strip()  # e.g. train/n004332/0317_01.jpg
...
#                     break
    ...
    def __getitem__(self, index):
        ...
        #print(f'img after crop w, h : {img.shape}')
